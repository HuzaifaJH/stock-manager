        ...
        try {
            const res = await fetch(
                selectedPurchase?.id ? `/api/purchases/${selectedPurchase.id}` : "/api/purchases",
                {
                    ...
                    body: JSON.stringify({ supplierId, date, items: purchaseItems }),
                });
            if (res.ok) {
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                ...
                <button className="btn btn-primary" onClick={() => setSelectedPurchase({ id: 0, supplierId: supplierId, date: date })}>
                    Add Purchase
                </button>
            </div>
                                ...
                                <tbody>
                                    {purchaseItems?.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>${(item.quantity || 0) * (item.purchasePrice || 0)}</td>
                                            {!viewMode && (
                                                <td>
